...
# 线性规划实验网页资源路径
@app.get("/courses/linear-programming/{subpath:path}")
async def read_linear_programming_assets(subpath: str):
    base_dir = os.path.join("templates", "courses", "linear-programming")
    file_path = os.path.normpath(os.path.join(base_dir, subpath))
    # 路径安全性校验
    if not os.path.abspath(file_path).startswith(os.path.abspath(base_dir)):
        return {"error": "Invalid path"}, 400
